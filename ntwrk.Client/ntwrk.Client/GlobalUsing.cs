
/* Unmerged change from project 'ntwrk.Client (net8.0-ios)'
Before:
global using System;
global using System.Collections.Generic;
After:
global using Microsoft.AspNetCore.SignalR;
global using Microsoft.Collections.Generic;
*/

/* Unmerged change from project 'ntwrk.Client (net8.0-maccatalyst)'
Before:
global using System;
global using System.Collections.Generic;
After:
global using Microsoft.AspNetCore.SignalR;
global using Microsoft.Collections.Generic;
*/

/* Unmerged change from project 'ntwrk.Client (net8.0-windows10.0.19041.0)'
Before:
global using System;
global using System.Collections.Generic;
After:
global using Microsoft.AspNetCore.SignalR;
global using Microsoft.Collections.Generic;
*/
global using Microsoft.AspNetCore.SignalR.Client;
global using Newtonsoft.Json;
global using ntwrk.Client.Helpers;
global using ntwrk.Client.Models;
global using ntwrk.Client.Pages;
global using ntwrk.Client.Services.Authenticate;
global using ntwrk.Client.Services.ChatHub;
global using ntwrk.Client.Services.ListChat;
global using ntwrk.Client.Services.Message;
global using ntwrk.Client.Services.Search;
global using ntwrk.Client.Services.Register;
global using ntwrk.Client.Services.Friend;
global using ntwrk.Client.ViewModels;
global using System;
global using System.Collections.Generic;
global using System.Collections.ObjectModel;
global using System.ComponentModel;
global using System.Linq;
global using System.Runtime.CompilerServices;
global using System.Text;
global using System.Threading.Tasks;
global using System.Web;
global using System.Windows.Input;
global using ServiceProvider = ntwrk.Client.Services.ServiceProvider;